...
class BaseConfig(BaseModel, Mapping):
    ...
    @classmethod
    def load_config(cls, namespace: str | None = None):
        """加载配置。
...
class BotConfig(BaseConfig):
    ...
    """mio 的 HTTP 和 WebSocket 服务端监听的 IP/主机名"""

    port: int = Field(default=7000, ge=1, le=65535)
    """mio 的 HTTP 和 WebSocket 服务端监听的端口"""
    ...
    """
    配置 mio 日志输出等级，可以为 `int` 类型等级或等级名称，
